...
input_directory = f"{upper_dir}/bowmanlab/borowsky.jonathan/FAST-cs/new_pockets"
...
pairs_all = sorted(np.load(f"{input_directory}/iofiles/output_indices/forward_lowrmsd_ligand_pairs_v{serial}.npy", allow_pickle = True), key = itemgetter(itemgetter_sort_ind), reverse = True)
...
def save(apo_holo_lig):
    ...
    pdb_output_dir = "../apo_structures_clean_2"
    npy_output_dir = "../pair_indices_2"
...
#beware that this script saves files locally with respect to the directory one is in on the pymol command line
