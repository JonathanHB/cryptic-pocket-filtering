...
def makedirectories():
    directories = ["moad_xml", "rcsb_pdb_holo", "rcsb_pdb_apo", "monomer_apo", "monomer_holo", "pairing_index", "warning_files"]
...
def checkligs(holo_xtal, idh, chh, apo_xtal, ida, cha, holo_ligand_iis):
    ...
    apo_calphas.save_pdb(f"./{ida}_chain{cha}_ca.pdb")
    holo_calphas.save_pdb(f"./{idh}_chain{chh}_ca.pdb")
    ...
    for i in holo_ligand_list: #run through holo candidate ligands
        ...
        if holo_ligand_iis == {}: #calculate holo ligand/pocket indices if none were provided
            ...
            holo_coords = holo_xtal.xyz[0][holo_xtal.top.select(f"resname '{i[0]}'")] #add ligand coordinates; [0] is frame of mdtraj trajectory;
            ...
            if include_resis: #if ligand-coordinating residues are included in distance calculations
                ...
                ligands = holo_xtal.top.select(f"resname '{i[0]}'")
                protein = holo_xtal.top.select("protein")
        ...
        #print(i)
        if ligdists(apo_ligand_list, apo_xtal, holo_coords, i):
             okay_ligands.append(i)

    return [okay_ligands, all_holo_coords, pair_rmsd] #if no issues are found
...
def ligdists(apoligandlist, apo_xtal, holo_coords, i):
    for j in apoligandlist: #loop through all nonstructural ligands in the apo structure
        apo_coords = apo_xtal.xyz[0][apo_xtal.top.select(f"resname '{j[0]}'")]
        #print(j)
...
def refine_ligand_list(idh, chh):
    ...
    for i in range(len(ligandlist)):
        ...
        if len(lig_a[0].split(" ")) > pept_threshold:
            ...
            print("removing peptide or other polymeric ligand")
        elif lig_a[0] in aa_resns:
            ligandlist.remove(lig_a)
            shift+=1 #avoid skipping elements and running off the end since indices change; just changing i doesn't work since it skips 0
            print("removing amino acid ligand")
...
blast_out = os.listdir(f"{directory}/blast_output/") #filenames for blastp results of candidate holo (MOAD) structures with at least one candidate apo (PDB blastp hit) structure not itself from MOAD
